...
The objective of the [estatjp]{.title-ref} Python package is to provide
access to the e-Stat portal and return datasets in
[pandas.DataFrame]{.title-ref} format.

For example, the e-Stat API returns CSV streams that contain headers
with metadata. These headers interfere with
[pandas.get_csv]{.title-ref}. The first release of estatjp returns a
dictionary that contains the header and main table as separate
dataframes.
...
for that table is assigned to [enurl]{.title-ref} below.
