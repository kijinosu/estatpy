---
csl: apa.csl
nocite: \[@\*\]
references:
- author:
  - family: Ashikawa
    given: Souta
  - family: Matsuda
    given: Junichi
  - family: Osone
    given: Tadashi
  container-title: Proceedings of annual conference of the information
    systems society in japan ISSJ2022
  doi: 10.19014/proceedingsissj.18.0_S1-C1
  id: ashizawa2022estat.en
  issued: 2022
  page: S1-C1
  publisher: Information Systems Society of Japan
  title: Method for improving the recall in e-stat data search
  type: paper-conference
- author:
  - family: Ashikawa
    given: Souta
  - family: Matsuda
    given: Junichi
  - family: Osone
    given: Tadashi
  container-title: Proceedings of annual conference of the information
    systems society in japan ISSJ2023
  doi: 10.19014/proceedingsissj.19.0_P001
  id: ashizawa2023estat.en
  issued: 2023
  page: 1-6
  publisher: Information Systems Society of Japan
  title: Development of front-end search system improving recall in
    e-stat
  type: paper-conference
- author:
  - literal: cocosan
  id: cocosan2023python.en
  issued: 2023-09-12
  publisher: "<https://www.youtube.com/watch?v=hiaK-jTXpCI>"
  title: Python apuri seifu tokei e-stat wo shigoto ni ikase!
- author:
  - literal: National Statistics Center, Japan
  id: estat2016adaptor
  issued: 2016-05-20
  publisher: "<https://github.com/e-stat-api/adaptor>"
  title: Chukan apuri
- abstract: The results of government statistics are provided centrally
    in the "portal site of official statistics of japan (e-Stat)",
    mainly on files such as Excel. However, there are many Excel files
    based on traditional reports, which makes it difficult to extract
    data. To solve this problem, we have proceeded with database
    creation and also provided API function that can obtain data
    directly from the program. Furthermore, to make the utilization of
    statistical data even more advanced, we started offering Linked Open
    Data (LOD) which is said to be the best in open data in 2016. In
    this paper, we will explain the current state of data provision of
    government statistics and explain how to provide statistical data in
    LOD format as well as its design and the usage.
  author:
  - family: Nishimura
    given: Shoki
  container-title: Joho Kanri
  doi: 10.1241/johokanri.59.812
  id: nishimura2017linked.en
  issue: 12
  issued: 2017
  page: 812-821
  publisher: Japan Science and Technology Agency
  title: "Providing statistical data by linked open data (LOD):
    Innovative official statistical data (e-stat) dissemination"
  title-short: Providing statistical data by linked open data (LOD)
  type: article-journal
  volume: 59
- author:
  - family: Kato
    given: Haruka
  - family: Takizawa
    given: Atsushi
  container-title: Sustainability
  doi: 10.3390/su132313413
  id: kato2021residential
  issue: 23
  issued: 2021
  page: 13413
  publisher: MDPI
  title: Which residential clusters of walkability affect future
    population from the perspective of real estate prices in the osaka
    metropolitan area?
  type: article-journal
  volume: 13
- author:
  - family: Seki
    given: Katsunori
  container-title: Social Science Japan Journal
  doi: 10.1093/ssjj/jyac029
  id: seki2023social
  issue: 1
  issued: 2023
  page: 47-60
  publisher: Oxford University Press UK
  title: "Social identification and redistribution preference: A survey
    experiment in japan"
  title-short: Social identification and redistribution preference
  type: article-journal
  volume: 26
- author:
  - family: Higashi
    given: Takahiro
  - family: Kurokawa
    given: Yukinori
  container-title: Annals of Gastroenterological Surgery
  doi: 10.1002/ags3.12835
  id: higashi2024incidence
  issue: 6
  issued: 2024
  page: 958-965
  publisher: Wiley Online Library
  title: Incidence, mortality, survival, and treatment statistics of
    cancers in digestive organs—japanese cancer statistics 2024
  type: article-journal
  volume: 8
- author:
  - family: Wakabayashi
    given: Chihiro
  - family: Shinmura
    given: Hiromi
  - family: Ando
    given: Miri
  - family: Shimada
    given: Masako
  - family: Yanagawa
    given: Hiroshi
  container-title: Gekkan Chiiki Igaku
  doi: 10.60261/chiikiigaku.29.2_52
  id: wakabayashi2015public.en
  issn: 09144277,27589196
  issue: 2
  issued: 2015
  page: 52
  publisher: Japan Association for the Development of Community Medicine
  title: "Kōeisei topikksu dai 13 kai seifutōkei no sōgōmadoguchi
    e-stat: Chiiki shindan he no katsuyō - jissen herusu puromōshon"
  title-short: Kōeisei topikksu dai 13 kai seifutōkei no sōgōmadoguchi
    e-stat
  type: article-journal
  volume: 29
- author:
  - family: Takahashi
    given: Shūichiro
  collection-title: Gijutsu no izumi series
  id: takahashi2022estat.en
  isbn: 9784295601050,4295601055
  issued: 2022
  publisher: Impress R&D
  title: "E-stat to nakayokusuru hon: Python to ōpun deta de nihon wo
    bunseki suru! API keiyu de seifu tōkei wo shutoku! katsuyo!"
  title-short: E-stat to nakayokusuru hon
  type: book
- author:
  - family: Masui
    given: Toshikatsu
  id: masui2021r.en
  isbn: 9784274227059
  issued: 2021
  publisher: Ohmsha
  title: R to python de manabu tokeigaku nyumon
  type: book
---

<div id="refs" class="references csl-bib-body hanging-indent"
entry-spacing="0" line-spacing="2">

<div id="ref-ashizawa2022estat.en" class="csl-entry">

Ashikawa, S., Matsuda, J., & Osone, T. (2022). Method for improving the
recall in e-stat data search. *Proceedings of Annual Conference of the
Information Systems Society in Japan ISSJ2022*, S1–C1.
<https://doi.org/10.19014/proceedingsissj.18.0_S1-C1>

</div>

<div id="ref-ashizawa2023estat.en" class="csl-entry">

Ashikawa, S., Matsuda, J., & Osone, T. (2023). Development of front-end
search system improving recall in e-stat. *Proceedings of Annual
Conference of the Information Systems Society in Japan ISSJ2023*, 1–6.
<https://doi.org/10.19014/proceedingsissj.19.0_P001>

</div>

<div id="ref-cocosan2023python.en" class="csl-entry">

<span class="nocase">cocosan</span>. (2023). *Python apuri seifu tokei
e-stat wo shigoto ni ikase!*
<https://www.youtube.com/watch?v=hiaK-jTXpCI>.

</div>

<div id="ref-higashi2024incidence" class="csl-entry">

Higashi, T., & Kurokawa, Y. (2024). Incidence, mortality, survival, and
treatment statistics of cancers in digestive organs—japanese cancer
statistics 2024. *Annals of Gastroenterological Surgery*, *8*(6),
958–965. <https://doi.org/10.1002/ags3.12835>

</div>

<div id="ref-kato2021residential" class="csl-entry">

Kato, H., & Takizawa, A. (2021). Which residential clusters of
walkability affect future population from the perspective of real estate
prices in the osaka metropolitan area? *Sustainability*, *13*(23),
13413. <https://doi.org/10.3390/su132313413>

</div>

<div id="ref-masui2021r.en" class="csl-entry">

Masui, T. (2021). *R to python de manabu tokeigaku nyumon*. Ohmsha.

</div>

<div id="ref-estat2016adaptor" class="csl-entry">

National Statistics Center, Japan. (2016). *Chukan apuri*.
<https://github.com/e-stat-api/adaptor>.

</div>

<div id="ref-nishimura2017linked.en" class="csl-entry">

Nishimura, S. (2017). Providing statistical data by linked open data
(LOD): Innovative official statistical data (e-stat) dissemination.
*Joho Kanri*, *59*(12), 812–821.
<https://doi.org/10.1241/johokanri.59.812>

</div>

<div id="ref-seki2023social" class="csl-entry">

Seki, K. (2023). Social identification and redistribution preference: A
survey experiment in japan. *Social Science Japan Journal*, *26*(1),
47–60. <https://doi.org/10.1093/ssjj/jyac029>

</div>

<div id="ref-takahashi2022estat.en" class="csl-entry">

Takahashi, S. (2022). *E-stat to nakayokusuru hon: Python to ōpun deta
de nihon wo bunseki suru! API keiyu de seifu tōkei wo shutoku! katsuyo!*
Impress R&D.

</div>

<div id="ref-wakabayashi2015public.en" class="csl-entry">

Wakabayashi, C., Shinmura, H., Ando, M., Shimada, M., & Yanagawa, H.
(2015). Kōeisei topikksu dai 13 kai seifutōkei no sōgōmadoguchi e-stat:
Chiiki shindan he no katsuyō - jissen herusu puromōshon. *Gekkan Chiiki
Igaku*, *29*(2), 52. <https://doi.org/10.60261/chiikiigaku.29.2_52>

</div>

</div>
